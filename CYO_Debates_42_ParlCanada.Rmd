...
This report is the output the final project from the Professional Certificate in Data Science HarvardX.  The final project is an analysis of data as selected by the course participant.  The focus for this report is text analytic of debates of the 42nd Parliament of Canada.  Analysis of the debates was selected to demonstrate comprehension of course material while using a less common data set.

Although the debates of the 42nd Parliament of Canada are available for download in a .csv format the step was taken to build a script to scrape the data from ourcommons.ca website.  Creation of the script demonstrates data collection from source materials, inclusion of key elements in the data not available in the curated data sets and specifying date ranges for data to be collected.  The script used for data collection is included as an Appendix to this report.

The analysis establishes closeness in the spoken content of the debate participants, positive sentiments outweighing negative sentiments in debates, the frequency of spoken words by party affiliation, and a moderate success of supervised categorization of text content by party affiliation.

\newpage
# Methods/Analysis
## Data Exploration
### Detect and Handle Anomalies
A 15% random sample of the data set provides 22,509 records of the text of people speaking in the debates.  Assessing the number of records by party will provide a sense of how often individuals are speaking in the debates.  An anomaly is noticed where the person speaking affiliation is assigned to an individual, not the party they are affiliated.  Closer inspection of the text reveals this is a special circumstance where the person speaking is nominating a person to function as speaker of the house for the purposes of electing a permanent speaker for the Parliament.  
...
debatecounts <- debates %>%
  ...
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = T, font_size = 10, latex_options = "hold_position")
```
Removing the special circumstance record the summary of speakers by party shows those participating in debates are dominated by the 1st (government), 2nd(official opposition) and 3rd (the 3rd party) parties making up the membership of the hours of commons.  Also observed, quantitatively, the minor parties participate in debates a much lesser degree than major parties.
\hfill\break
...
With an identifier for the Parliament it is possible to see how many speakers were recorded by year.  It is quickly observers the data sample is heavily weighted to the 42nd Parliament.  For the purposes of this analysis that is fortunate occurrence.  If other Parliaments were of interest for analysis, the data collection script in Appendix A can be modified to target specific parliaments by augmenting the URL "&ParlSes=All" to reflect the Parliament of interest.  For example replacing 'All' with 39 (=L&Item=&ParlSes=39) to collect debates only from the 39th Parliament of Canada.
```{r, debats.year, echo=FALSE, include=T, results='hold', warning=F, message=F}
    debates %>%
      group_by(parl) %>%
      count(sort = T) %>%
      ungroup() %>%
  kable(caption = "debate by year", booktabs = T) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F, font_size = 10, latex_options = "hold_position")
```

We can explore who speaks most.  Is there an individual who dominates the debates in the House of Commons?
...
As a final step in our data exploration and clean up we can select only those fields of interest for analysis from the set of debates records.  This will help with processing the text in quantitative analysis steps by eliminating non value add items for this analysis such as the ID and date of the document from where text was collected, the date of the document ID and the time stamp of the when the person was recorded as speaking in the House of Commons.

In the following section, Quantitative Analysis with the Quanteda, the advantage of the Quanteda's `readtext` function in working with text data will be explained. To format the debate data for `readtext` the selected data is written locally as tab seperated values file CYO.Debates.tsv
...
* Tokens extracted from the corpus
* Document Feature Matrix representing frequencies of features in documents in a matrix

The data to be used for analysis will read with Quanteda's readtext package.  Using readtext provides the advantage of using a fucntion for reading the data that has been developed escpecially for Quanteda.  The readtext package also provides the convenience of identifying the 'text' for analysis and document level variables (docvars) that will be used in grouping the data in later stages of text analysis. The corpus is created using Quanteda's corpus command
\hfill\break
...
As the proportion of the total records collected from ourparliament.ca being used for analysis is 15% of the total records collected the Training set has been assigned as 90% of the records for anlysis and 10% assigned to test.  The 90/10 split has been used to provide adequate records for training planned classification of documents.  

A repoducable random sample records ids comprising of 10% of total number of observations in corpus_debates is assigned to vector id_test.  Creation of corp_debates$id_numeric will permit for the matching test_ids to create a corpus of test records.  By creating a corpus of unmatched records in id_test the training corpus is created.  The creation of document frame matrix in the same chunk of code provides a final output of training and test corpus and document frame matrix.  The corpus and dfms will be used for the majority of the analysis completed from this point forward.  The test corpus is excelusively for use as final validation of the classification algorithym.
...
#### Frequency Analysis
##### WordCloud  
A word cloud or text cloud is a simple and effective way to present the frequency of words as they appear in the debates text. The bigger and bolder the word appears, the more often it’s mentioned within the debate text.  In analysis of the debates of the 42nd Parliament of Canada the word cloud provides a view to the words most often used in the house during debates.  Not surprisingly, the words shown in the analysis predominatly are House terms such as member, Candadians, committee etc.  There are intersting terms showing in the cloud such as indigenous, economic, support and health.  These words used repeatidly during the 42nd Pariliament over a period of nearly 4 years is an insight to what is being said or mentioned in the course of debates in The House of Commons.
...
The inclusion of docvars in the creation of the corpus enables grouping terms by any of the docvars present in the corpus.  The debates_corpus includes docvars recording personSpeaking, personSpeakingAffilliation and the personSpeakingDate.  Applying the personSpeakingAffiliation docvar the most frequent terms spoken in debates can be plotted by the party the person speaking is affilliated. The dfm_weght function has an argument to measure the dfm results as the proportion of the feature counts of total feature counts.  In words, provide the relative frequency calculated as   $\sum_{ij} / \sum_j tf_{ij}$

The resulting dfm, when used with textstat_frequency argument groups being personSpeakingAffiliation can be plotted to show the frequency of terms in faceted by party affilliation.  
...
freq_weight <- textstat_frequency(dfm_debates_pty_prop, n = 7, groups = "personSpeakingAffiliation")
...
The document feature matrix can also be used to show the closness of terms used by those persons speaking in debates.  The closness of terms used in speaking is then able to shown in clusters visually in a Cluster Dendrogram.  Interesting groupings are revealed in the resulting plot.  For example, Green Party leader Elizabeth May is clustered with New Democratic Party member Murray Rankin.  Of particular interest is they are close to Bardish Chagger who is a ranking member of the Liberal government as a house leader.  To the other extreme Conservative Party of Canada member Garnett Genuis is clustered with Liberal Kevin Lamoureux who we idenffied as a Parlimentary Secretary appointed by the Prime Minister.  It should be noted the Dendrogram will can be refined or expanded based on the arguments controlling the minimum number of characters needed to be considered a word and the minimum and maximum number of times the terms being cluster must occur accross all documents in the dfm. 
...
# Subset corpus to texts of the house and house opposition leaders
    corp_debates_leaders <- corpus_subset(corp_training,
                                       personSpeaking == c("Hon. Dominic LeBlanc", "Hon. Andrew Scheer", 
                                                           "Hon. Bardish Chagger","Hon. Candice Bergen")) 
    ...
    freq_weight_leaders <- textstat_frequency(dfm_debates_leaders_prop, n = 15, groups = c("personSpeakingAffiliation", "personSpeaking"))
...
Proportionaly there are terms that are common accross all 4 House leaders in the 42nd Parliament.  The kwic (key words in context) function will present the term with words before (pre token) and words after (post token).  Including workds before and after the token gives the usage of the term context.  For example the term government is frequently used by each of the leaders. Including the 5 words before and after the term gives context to how person speaking applied the term at a specefic point in the debate.  
...
The Naive Bayes classifier classifies based on probabilities of events. As the debate coprus has an associated docvar of personSpeakingAffiliation (i.e. the political party they are a member) the feature can be used test if the debate texts can classified by Party Affiliation.  The algorithym for the classification test will be accessed via Quanteda's textmodel_nb().  The results of classifier are then used in the creation of a Confusion Matrix that will lend it self to plotting a heat map of results of the classification test.  


```{r, NBdocclsfctn, echo=T, include=F, results='markup', warning=F, message=F}
...
The results of the classification are summarised as
...
An alternative presentation of the results table is a heat map.  We clearly see where the successes in classifying occured as distinct colours where the predicted class intersects with the actual class from the document feature matrix applied in the Naive Bayes test.
```{r, cmheatmap, echo=T, include=T, results='markup', warning=F, message=F}
...
```
The validation of the classifier with the test set provides an much improved result over the training set as is seen from the following summary rows from the two confusion matrix.
...
#### Sentiment Analysis
What text analysis of debates would be complete without a sentiment analysis of the texts analyzed? The table of results speaks for it self revealing positive attributes are measured more frequently that negative ones in the words used by the persons speaking in the debates. 
...
Debates by nature are series words to make, defend and challenge points and counter points. Analysis of the text of the debates of 42nd Parliament of Canada reveals the words used in the course of debate are often used in related contexts regardless of party affiliation of the debator.  However, as shown in hiararichal clustering, there are patterns of closeness within party affiliation and the words used during debates.  It is also demonstrated in the confusion matrix the affilliation of major parties of the member speaking in the debate can be classified to a certain extent.  Finally the analysis demonstrates even less frequent participants in debates from minor parties have closness with the major parties.  

# Conclusion
This report demonstrates text analytics can be applied in areas as diverse as debates in a political arena to extract intersting insights into the words used in the course of debating the formation and refinements of the nations laws and public policy.  However, this report should not be considered a determining insight into the work of the elected officials participating in the debates.  Much of the policy work in government is done in committee where members have a focused opportunity to apply their wants, wishes and needs in formating policy.  Analysis of the committee proceedings is a next logical step in assessing text analytics application in quantifying debate as influencers in policy.
